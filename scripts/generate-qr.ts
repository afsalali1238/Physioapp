...
async function generateQRCodes() {
  ...
  if (!fs.existsSync(areasPath)) {
    console.error('areas.json not found. Run sync first.');
    process.exit(1);
  }

  const areas = JSON.parse(fs.readFileSync(areasPath, 'utf8'));
  const publishedAreas = areas.filter((a: any) => a.status === 'published');
  
  const sheetItems: string[] = [];

  for (const area of publishedAreas) {
    const url = `${BASE_URL}/${area.section}/${area.area_id}`;
    const filename = `${area.section}-${area.area_id}.png`;
