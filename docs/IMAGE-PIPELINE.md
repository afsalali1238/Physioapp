...
## Interim: drawn SVG figures in the prototype

The review prototype ships **nine hand-drawn SVG figures**, one per neck item. They are written as code in `prototype/patient-library-prototype.html`, not licensed or generated, so there is no rights question to defer.

**How they work.** One figure system — front, side-seated, back, and wall — reused across every pose, with the head as a transformable group. That is why the set is consistent: it is the same figure, not nine attempts at the same figure. A tinted band marks where the patient should feel the exercise, coloured by the item's type (mobility / stretch / strengthen).

**What they are for.** Confirming the card layout, the image aspect ratio, and the "show where you feel it" idea before any money is spent. They are a style reference, not production art.

**One thing they already proved.** The first draft tilted the head toward the viewer's right for "let your right ear drop towards your right shoulder" — anatomically backwards, because a front-facing figure's right is the viewer's left. That error would have been baked into a licensed illustration order or an AI batch just as easily. **Every image gets checked against its instruction text for left/right correctness**, and the physiotherapist signs it off. It is now the second question in the review gate below.

**Known gaps in the drawn set:** the wall cue in `ex-neck-05` reads as "near a wall" rather than "back against a wall", and the figures are deliberately schematic — no hands, no clothing detail. Production art should fix both.
...
2. **Left/right check.** Does the side shown match the instruction text? A front-facing figure's right is the viewer's left. This is the error that slips through most easily.
3. Does the depicted position match the written instruction beside it, exactly?
4. Is any prop stable and safe to lean on? *(A published review criticised Physitrack's imagery for showing patients bracing on flimsy chairs. Clinicians judge whether the setup is replicable, not whether it looks nice.)*
5. Consistent with the rest of the set — same figure treatment, palette, background, camera height?
6. Clothing full-coverage, setting domestic rather than gym?
7. Does it read at 400px wide on a phone?
