// Parametric body skeleton. Every silhouette part and every region hotspot is
// derived from the SAME joint table, so hotspots cannot drift off the body.
// Coordinate space: viewBox "0 0 240 620". Centre line x = 120.
// Proportions: 8-head canon. head top y=20, chin y=92, sole y=582.

export const VIEWBOX = '0 0 240 620';
export const CX = 120;

// --- joints -----------------------------------------------------------------
// Named in ANATOMICAL terms. "l"/"r" are the PATIENT's left/right.
...
// --- helpers ----------------------------------------------------------------
export const cap = (a, b) => `M${a[0]} ${a[1]} L${b[0]} ${b[1]}`;

/** Mid-point, optionally biased t along a→b. */
export const lerp = (a, b, t = 0.5) => [
  a[0] + (b[0] - a[0]) * t,
  a[1] + (b[1] - a[1]) * t,
];

// --- silhouette -------------------------------------------------------------
// Drawn as round-capped strokes plus two filled shapes. Same fill colour, so
// the overlaps read as one body.
export const SILHOUETTE = {
  filled: [
    ...
    `M74 134 C96 124 144 124 166 134
     L170 182 L158 238 L162 268 L160 300
     C150 314 90 314 80 300
     L78 268 L82 238 L70 182 Z`,
  ],
  strokes: [
...
// Interior detail lines — purely decorative, differ front vs back so the two
// views are visually distinguishable at a glance.
export const DETAIL = {
  front: [
