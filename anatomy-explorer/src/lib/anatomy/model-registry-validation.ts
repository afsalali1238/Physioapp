...
export function validateModelRegionVisuals(
  ...
  for (const visual of visuals) {
    ...
    if (!regions.has(visual.regionId)) errors.push(`Visual regionId "${visual.regionId}" has no published-region record.`);
    for (const meshName of [...visual.meshNames, ...visual.hitMeshNames]) {
      if (!meshes.has(meshName)) errors.push(`Visual region "${visual.regionId}" references missing mesh "${meshName}".`);
    }
  }
  return errors;
}
