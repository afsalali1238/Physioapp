...
function sided(side: 'l' | 'r'): GeometryRegion[] {
  ...
  return [
    {
      id: `shoulder-${side}`, areaId: 'shoulder', views: ['front', 'back'], side,
      label: `${s} shoulder`, zones: SHOULDER_ZONES,
      shapes: [{ d: cap(p.sh, A(p.sh, p.el, 0.24)), w: 40 }],
    },
    {
      id: `elbow-${side}`, areaId: 'elbow', views: ['front', 'back'], side,
      label: `${s} elbow`, zones: ELBOW_ZONES,
      shapes: [{ d: cap(A(p.sh, p.el, 0.86), A(p.el, p.wr, 0.16)), w: 34 }],
    },
    {
      id: `wrist-${side}`, areaId: 'wrist', views: ['front', 'back'], side,
      label: `${s} wrist`, zones: WRIST_ZONES,
      shapes: [{ d: cap(A(p.el, p.wr, 0.88), p.wr), w: 30 }],
    },
    {
      id: `hip-${side}`, areaId: 'hip', views: ['front', 'back'], side,
      label: `${s} hip`, zones: HIP_ZONES,
      shapes: [{ d: cap(p.tr, A(p.hip, p.kn, 0.1)), w: 44 }],
    },
    {
      id: `knee-${side}`, areaId: 'knee', views: ['front', 'back'], side,
      label: `${s} knee`, zones: KNEE_ZONES,
      shapes: [{ d: cap(A(p.hip, p.kn, 0.9), A(p.kn, p.an, 0.11)), w: 44 }],
    },
    {
      id: `ankle-${side}`, areaId: 'ankle', views: ['front', 'back'], side,
      label: `${s} ankle`, zones: ANKLE_ZONES,
...
export const GEOMETRY_REGIONS: readonly GeometryRegion[] = [
  {
    id: 'neck', areaId: 'neck', views: ['front', 'back'], side: 'c',
    label: 'Neck', zones: NECK_ZONES,
    shapes: [{ d: cap([120, 102], [120, 126]), w: 40 }],
  },
  {
    // BACK ONLY — see the header. Do not give this a front path.
    id: 'lower-back', areaId: 'lower-back', views: ['back'], side: 'c',
    label: 'Lower back', zones: LOWBACK_ZONES,
