...
export function getApprovedMotion(exerciseId: string): MotionAsset | undefined {
  return MOTION_ASSETS.find((asset) => asset.exerciseId === exerciseId && asset.status === 'approved');
}
...
export function getPreviewMotion(exerciseId: string): MotionAsset | undefined {
  return MOTION_ASSETS.find((asset) => asset.exerciseId === exerciseId && asset.status !== 'retired');
}
